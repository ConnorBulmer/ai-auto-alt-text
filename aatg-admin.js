    ...
    jQuery.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                var message = 'Alt text generated: <em>' + newAlt + '</em>. Please refresh the page to update the input field.';
                if (warning) {
                    message += ' <strong>Note:</strong> ' + warning;
                }
        ...
        error: function(xhr, status, error) {
            ...
            button.prop('disabled', false);
            button.siblings('.aatg-result').html(
                '<strong>Error:</strong> ' + (error || 'Request failed.')
            );
            console.log(error);
        }
    });
